...
async function createJWT(algorithm, key, payloadClaims, options) {
  ...
  const headerPart = import_encoding.base64url.encode(textEncoder.encode(JSON.stringify(header)), {
    includePadding: false
  });
  const payloadPart = import_encoding.base64url.encode(textEncoder.encode(JSON.stringify(payload)), {
    includePadding: false
  });
  const data = textEncoder.encode([headerPart, payloadPart].join("."));
  const signature = await getAlgorithm(algorithm).sign(key, data);
  const signaturePart = import_encoding.base64url.encode(new Uint8Array(signature), {
    includePadding: false
  });
...
async function validateJWT(algorithm, key, jwt) {
  ...
  if (parsedJWT.expiresAt && !(0, import__.isWithinExpirationDate)(parsedJWT.expiresAt)) {
    throw new Error("Expired JWT");
  }
  if (parsedJWT.notBefore && Date.now() < parsedJWT.notBefore.getTime()) {
    throw new Error("Inactive JWT");
  }
  const signature = import_encoding.base64url.decode(parsedJWT.parts[2], {
    strict: false
  });
...
function parseJWT(jwt) {
  ...
  const rawHeader = import_encoding.base64url.decode(jwtParts[0], {
    strict: false
  });
  const rawPayload = import_encoding.base64url.decode(jwtParts[1], {
    strict: false
  });
  ...
  if ("exp" in payload) {
    ...
    properties.expiresAt = new Date(payload.exp * 1e3);
  }
  if ("iss" in payload) {
  ...
  if ("nbf" in payload) {
    ...
    properties.notBefore = new Date(payload.nbf * 1e3);
  }
  if ("iat" in payload) {
    if (typeof payload.iat !== "number") {
      return null;
    }
    properties.issuedAt = new Date(payload.iat * 1e3);
  }
  if ("jti" in payload) {
...
function getAlgorithm(algorithm) {
  if (algorithm === "ES256" || algorithm === "ES384" || algorithm === "ES512") {
    return new import_crypto.ECDSA(ecdsaDictionary[algorithm].hash, ecdsaDictionary[algorithm].curve);
  }
  if (algorithm === "HS256" || algorithm === "HS384" || algorithm === "HS512") {
    return new import_crypto.HMAC(hmacDictionary[algorithm]);
  }
  if (algorithm === "RS256" || algorithm === "RS384" || algorithm === "RS512") {
    return new import_crypto.RSASSAPKCS1v1_5(rsassapkcs1v1_5Dictionary[algorithm]);
  }
  if (algorithm === "PS256" || algorithm === "PS384" || algorithm === "PS512") {
    return new import_crypto.RSASSAPSS(rsassapssDictionary[algorithm]);
  }
